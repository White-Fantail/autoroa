...
describe("admin page", () => {
  ...
  it("opens read-only details for completed and failed OCR jobs", async () => {
    auth.token="admin-token";
  ...
  it("uses the native action button as the row's only keyboard target", async () => {
    auth.token="admin-token";let mediaReads=0;const job={id:"button-detail",media_asset_id:"button-media",status:"READY",requires_confirmation:false,created_at:"2026-08-14T00:00:00Z",result_json:{prices:[]}};
    vi.mocked(fetch).mockImplementation(async input=>{const url=String(input);if(url.endsWith("/admin/dashboard"))return jsonResponse({users:1});if(url.includes("/ocr-jobs?kind=PRICE_BOARD"))return jsonResponse([job]);if(url.endsWith("/admin/stations"))return jsonResponse([]);if(url.endsWith("/admin/media/button-media/content")){mediaReads+=1;return {ok:true,blob:async()=>new Blob(["image"],{type:"image/jpeg"})} as Response}return jsonResponse({},404)});
    render(<Admin/>);fireEvent.click(await screen.findByRole("button",{name:"OCR Queue"}));const action=await screen.findByRole("button",{name:"View"});action.focus();expect(document.activeElement).toBe(action);expect(action.closest("tr")?.hasAttribute("tabindex")).toBe(false);fireEvent.keyDown(action,{key:"Enter"});action.click();fireEvent.keyUp(action,{key:"Enter"});expect(await screen.findByRole("heading",{name:"OCR job details"})).toBeTruthy();await waitFor(()=>expect(mediaReads).toBe(1));
  });
