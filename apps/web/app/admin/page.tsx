...
function AdminOCRQueue({token}:{token:string}){
  ...
  useEffect(()=>{void Promise.all([loadJobs(true),loadStations(true)]);if(review)return;const timer=setInterval(()=>void loadJobs(),5000);return()=>clearInterval(timer)},[token,review]);
...
function PriceBoardQueueReview({job,stations,stationLoading,stationError,onRetryStations,token,onCancel,onSaved}:{job:any;stations:any[];stationLoading:boolean;stationError:string;onRetryStations:()=>void;token:string;onCancel:()=>void;onSaved:()=>void}){
  ...
  const confidence=job.confidence==null?"Not available":`${Math.round(Number(job.confidence)*100)}%`;
  return <div className="admin-ocr-review"><button className="admin-back" type="button" onClick={onCancel}>← Back to queue</button><header className="admin-ocr-review-header"><div><p className="admin-kicker">OCR Queue</p><h2>Review extracted prices</h2><p>Choose the correct station, verify the extracted values, then apply them.</p></div><span className={`admin-status-badge admin-status-${String(job.status).toLowerCase().replaceAll("_","-")}`}>{humanizeField(job.status)}</span></header><div className="admin-ocr-review-layout"><PriceBoardImage mediaAssetId={job.media_asset_id} token={token}/><form className="admin-price-board admin-ocr-review-form" onSubmit={apply}><dl className="admin-ocr-summary"><div><dt>Uploaded</dt><dd>{new Intl.DateTimeFormat(undefined,{dateStyle:"medium",timeStyle:"short"}).format(new Date(job.created_at))}</dd></div><div><dt>Overall confidence</dt><dd>{confidence}</dd></div></dl><StationCombobox stations={stations} value={stationId} onChange={setStationId} loading={stationLoading} error={stationError} onRetry={onRetryStations}/><fieldset><legend>Extracted prices</legend><div className="admin-price-board-grid">{fuelTypes.map(type=>{const entry=(job.result_json?.prices??[]).find((item:any)=>item.fuel_type===type);return <label key={type}>{humanizeField(type)}{entry?.confidence!=null&&<span>{Math.round(Number(entry.confidence)*100)}% confidence</span>}<input type="number" min="0.001" max="20" step="0.001" value={prices[type]??""} onChange={event=>setPrices(current=>({...current,[type]:event.target.value}))} /></label>})}</div></fieldset>{error&&<p className="admin-alert" role="alert">{error}</p>}<div className="admin-form-actions"><button type="button" onClick={onCancel}>Back to queue</button><button className="admin-primary" disabled={saving||!stationId||!Object.values(prices).some(Boolean)} type="submit">{saving?"Applying…":"Confirm and apply"}</button></div></form></div></div>
}
