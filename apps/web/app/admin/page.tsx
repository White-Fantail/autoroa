...
function AdminOCRQueue({token}:{token:string}){
  ...
  async function refresh(){await Promise.all([loadJobs(),loadStations()])}
  useEffect(()=>{queueMounted.current=true;return()=>{queueMounted.current=false}},[]);
  useEffect(()=>{void Promise.all([loadJobs(true),loadStations(true)]);if(detail)return;const timer=setInterval(()=>void loadJobs(),5000);return()=>clearInterval(timer)},[token,detail]);
