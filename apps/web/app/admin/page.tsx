...
function AdminOCRQueue({token}:{token:string}){
  const [jobs,setJobs]=useState<any[]>([]);const [stations,setStations]=useState<any[]>([]);const [users,setUsers]=useState<any[]>([]);const [detail,setDetail]=useState<any>();const [uploading,setUploading]=useState(false);const [message,setMessage]=useState("");const [queueLoading,setQueueLoading]=useState(true);const [queueError,setQueueError]=useState("");const [stationLoading,setStationLoading]=useState(true);const [stationError,setStationError]=useState("");const queueMounted=useRef(true);
  async function loadJobs(showLoading=false){if(showLoading)setQueueLoading(true);try{const response=await fetch(`${api}/ocr-jobs?kind=PRICE_BOARD&limit=20`,{headers:{authorization:`Bearer ${token}`}});if(!response.ok)throw new Error();const next=await response.json();if(queueMounted.current){setJobs(next);setQueueError("")}}catch{if(queueMounted.current)setQueueError("The OCR queue could not be loaded. Try again.")}finally{if(queueMounted.current)setQueueLoading(false)}}
  async function loadStations(showLoading=false){if(showLoading)setStationLoading(true);try{const response=await fetch(`${api}/admin/stations`,{headers:{authorization:`Bearer ${token}`}});if(!response.ok)throw new Error();const next=await response.json();if(queueMounted.current){setStations(next);setStationError("")}}catch{if(queueMounted.current)setStationError("Stations could not be loaded. Try again.")}finally{if(queueMounted.current)setStationLoading(false)}}
  async function loadUsers(){try{const response=await fetch(`${api}/admin/users`,{headers:{authorization:`Bearer ${token}`}});if(!response.ok)return;const next=await response.json();if(queueMounted.current&&Array.isArray(next))setUsers(next)}catch{/* Uploader IDs remain available if profile lookup fails. */}}
  async function refresh(){await Promise.all([loadJobs(),loadStations(),loadUsers()])}
  useEffect(()=>{queueMounted.current=true;return()=>{queueMounted.current=false}},[]);
  useEffect(()=>{void Promise.all([loadJobs(true),loadStations(true),loadUsers()]);if(detail)return;const timer=setInterval(()=>void loadJobs(),5000);return()=>clearInterval(timer)},[token,detail]);
  ...
  const uploaderNames=Object.fromEntries(users.map(user=>[String(user.id),String(user.display_name||shortId(String(user.id)))]));
  if(detail)return <PriceBoardQueueDetail key={detail.id} initialJob={detail} stations={stations} uploaderNames={uploaderNames} stationLoading={stationLoading} stationError={stationError} onRetryStations={()=>void loadStations(true)} token={token} onCancel={async()=>{setDetail(undefined);await loadJobs()}} onSaved={async()=>{setDetail(undefined);await refresh()}} />;
  function openDetail(job:any){setDetail(job)}
  return <section className="admin-price-board admin-ocr-queue" aria-label="Price-board OCR queue"><header><div><h2>Price-board OCR queue</h2><p>Upload a board without choosing a station, or review jobs that need attention.</p></div><label className="admin-primary">{uploading?"Uploading…":"Upload unassigned photo"}<input hidden type="file" accept="image/jpeg,image/png,image/webp" disabled={uploading} onChange={event=>{const file=event.target.files?.[0];if(file)void uploadUnassigned(file);event.target.value=""}} /></label></header>{message&&<p role="status">{message}</p>}{queueError&&<div className="admin-inline-error" role="alert"><span>{queueError}</span><button type="button" onClick={()=>void loadJobs(true)}>Retry queue</button></div>}{stationError&&<div className="admin-inline-error" role="alert"><span>{stationError}</span><button type="button" onClick={()=>void loadStations(true)}>Retry stations</button></div>}<div className="admin-table-wrap">{queueLoading&&jobs.length===0?<p className="admin-empty" role="status">Loading OCR jobs…</p>:!queueError&&jobs.length===0?<p className="admin-empty">No recent price-board jobs.</p>:jobs.length>0?<table className="admin-table admin-ocr-table"><caption className="sr-only">Recent price-board OCR jobs</caption><thead><tr><th>Uploaded</th><th>Uploaded by</th><th>Station</th><th>Status</th><th>Confidence</th><th>Result</th><th><span className="sr-only">Actions</span></th></tr></thead><tbody>{jobs.map(job=><tr key={job.id} onClick={()=>openDetail(job)}><td>{formatJobDate(job.created_at)}</td><td>{formatUploader(job,uploaderNames)}</td><td>{job.station_id?stationNames[job.station_id]||job.station_id:<span className="admin-muted">Unassigned</span>}</td><td><span className={`admin-status-badge admin-status-${String(job.status).toLowerCase().replaceAll("_","-")}`}>{humanizeField(job.status)}</span></td><td>{job.confidence!=null?`${Math.round(Number(job.confidence)*100)}%`:"—"}</td><td>{job.status==="READY"||job.status==="CONFIRMED"?"Applied":job.status==="REVIEW_REQUIRED"?"Needs review":job.status==="FAILED"?job.error_message||"Failed":"Processing"}</td><td className="admin-ocr-action"><button type="button" onClick={event=>{event.stopPropagation();openDetail(job)}}>{job.status==="REVIEW_REQUIRED"?"Review":"View"}</button></td></tr>)}</tbody></table>:null}</div></section>
}

function PriceBoardQueueDetail({initialJob,stations,uploaderNames,stationLoading,stationError,onRetryStations,token,onCancel,onSaved}:{initialJob:any;stations:any[];uploaderNames:Record<string,string>;stationLoading:boolean;stationError:string;onRetryStations:()=>void;token:string;onCancel:()=>void;onSaved:()=>void}){
  const [job,setJob]=useState(initialJob);const [stationId,setStationId]=useState(initialJob.station_id??"");const [prices,setPrices]=useState<Record<string,string>>(()=>Object.fromEntries((initialJob.result_json?.prices??[]).map((entry:any)=>[entry.fuel_type,String(entry.price_per_litre)])));const [saving,setSaving]=useState(false);const [error,setError]=useState("");const pending=job.status==="UPLOADED"||job.status==="PROCESSING";const editable=job.status==="REVIEW_REQUIRED";
  useEffect(()=>{if(!pending)return;let active=true;async function update(){try{const response=await fetch(`${api}/ocr-jobs/${job.id}`,{headers:{authorization:`Bearer ${token}`}});if(!response.ok)throw new Error();const next=await response.json();if(active){setJob(next);setError("");if(next.status==="REVIEW_REQUIRED"){setStationId(next.station_id??"");setPrices(Object.fromEntries((next.result_json?.prices??[]).map((entry:any)=>[entry.fuel_type,String(entry.price_per_litre)])))}}}catch{if(active)setError("The latest job status could not be loaded.")}}void update();const timer=setInterval(()=>void update(),5000);return()=>{active=false;clearInterval(timer)}},[job.id,pending,token]);
  async function apply(event:FormEvent){event.preventDefault();if(!editable)return;const entries=fuelTypes.filter(type=>prices[type]?.trim()).map(type=>({fuel_type:type,price:prices[type]}));if(!stationId||!entries.length)return;setSaving(true);setError("");try{const response=await fetch(`${api}/admin/stations/${stationId}/price-board`,{method:"POST",headers:{authorization:`Bearer ${token}`,"content-type":"application/json"},body:JSON.stringify({job_id:job.id,media_asset_id:job.media_asset_id,observed_at:new Date(job.created_at).toISOString(),prices:entries})});if(!response.ok)throw new Error(adminMutationError(response.status));onSaved()}catch(caught){setError(caught instanceof Error?caught.message:"Could not apply prices.")}finally{setSaving(false)}}
  const confidence=job.confidence==null?"Not available":`${Math.round(Number(job.confidence)*100)}%`;const entries=job.result_json?.prices??[];const photoLocation=job.result_json?.photo_location;const selectedStation=stations.find(item=>item.id===job.station_id);const detectedStationId=photoLocation?.detected_station_id?String(photoLocation.detected_station_id):"";const detectedStation=stations.find(item=>item.id===detectedStationId);const showStationQuickActions=editable&&Boolean(job.station_id)&&Boolean(photoLocation?.station_mismatch);
  return <div className="admin-ocr-review"><button className="admin-back" type="button" onClick={onCancel}>← Back to queue</button><header className="admin-ocr-review-header"><div><p className="admin-kicker">OCR Queue</p><h2>{editable?"Review extracted prices":"OCR job details"}</h2><p>{editable?"Choose the correct station, verify the extracted values, then apply them.":pending?"Processing continues in the background. This page updates automatically.":"View the source, extracted values, and processing history."}</p></div><span className={`admin-status-badge admin-status-${String(job.status).toLowerCase().replaceAll("_","-")}`}>{humanizeField(job.status)}</span></header><div className="admin-ocr-review-layout"><PriceBoardImage mediaAssetId={job.media_asset_id} token={token}/><form className="admin-price-board admin-ocr-review-form" onSubmit={apply}><dl className="admin-ocr-summary"><JobFact label="Uploaded" value={formatJobDate(job.created_at)}/><JobFact label="Uploaded by" value={formatUploader(job,uploaderNames)}/><JobFact label="Started" value={formatJobDate(job.started_at)}/><JobFact label="Completed" value={formatJobDate(job.completed_at)}/><JobFact label="Applied" value={formatJobDate(job.applied_at)}/><JobFact label="Overall confidence" value={confidence}/><JobFact label="Station" value={selectedStation?.name||job.station_id||"Unassigned"}/><JobFact label="Photo location" value={formatPhotoLocation(photoLocation)}/><JobFact label="Station matching" value={formatLocationMatch(photoLocation)}/><JobFact label="Confirmation" value={job.requires_confirmation?"Required":"Not required"}/><JobFact label="Application" value={formatApplication(job)}/></dl>{photoLocation?.reason&&<p className="admin-inline-error" role="status"><span><strong>Location diagnostic:</strong> {photoLocation.reason}</span></p>}{showStationQuickActions&&<div aria-label="Station review shortcuts" style={{display:"grid",gap:8,padding:"12px",border:"1px solid var(--border)",borderRadius:10,background:"var(--surface)"}}><strong>Choose station</strong><div style={{display:"flex",gap:8,flexWrap:"wrap"}}><button type="button" className={stationId===String(job.station_id)?"admin-primary":""} onClick={()=>setStationId(String(job.station_id))}>Use selected: {selectedStation?.name||"selected station"}</button>{detectedStationId&&<button type="button" className={stationId===detectedStationId?"admin-primary":""} onClick={()=>setStationId(detectedStationId)}>Change to detected: {detectedStation?.name||photoLocation?.detected_station_name||"detected station"}</button>}</div></div>}{job.error_message&&<p className="admin-alert" role="alert"><strong>Processing error:</strong> {job.error_message}</p>}{editable&&<StationCombobox stations={stations} value={stationId} onChange={setStationId} loading={stationLoading} error={stationError} onRetry={onRetryStations}/>}<fieldset><legend>Extracted prices</legend>{entries.length===0&&!editable?<p className="admin-muted">No extracted prices are available yet.</p>:<div className="admin-price-board-grid">{(editable?fuelTypes:entries.map((entry:any)=>entry.fuel_type)).map((type:string)=>{const entry=entries.find((item:any)=>item.fuel_type===type);return <label key={type}>{humanizeField(type)}{entry?.confidence!=null&&<span>{Math.round(Number(entry.confidence)*100)}% confidence</span>}<input type="number" min="0.001" max="20" step="0.001" value={editable?prices[type]??"":String(entry?.price_per_litre??"")} readOnly={!editable} aria-readonly={!editable} onChange={editable?event=>setPrices(current=>({...current,[type]:event.target.value})):undefined} /></label>})}</div>}</fieldset>{error&&<p className="admin-alert" role="alert">{error}</p>}<div className="admin-form-actions"><button type="button" onClick={onCancel}>Back to queue</button>{editable&&<button className="admin-primary" disabled={saving||!stationId||!Object.values(prices).some(Boolean)} type="submit">{saving?"Applying…":"Confirm and apply"}</button>}</div></form></div></div>
}

function JobFact({label,value}:{label:string;value:string}){return <div><dt>{label}</dt><dd>{value}</dd></div>}
function formatJobDate(value?:string|null){if(!value)return "—";const date=new Date(value);return Number.isNaN(date.getTime())?"—":new Intl.DateTimeFormat(undefined,{dateStyle:"medium",timeStyle:"short"}).format(date)}
function formatUploader(job:any,uploaderNames:Record<string,string>){const source=String(job.submission_source??job.result_json?.submission_source??"").toUpperCase();const role=source==="FUEL_MAP_USER"?"User":source==="ADMIN"?"Admin":job.user_id?"Account":"User";const id=job.user_id?String(job.user_id):"";const name=id?uploaderNames[id]||shortId(id):"";return name?`${role} · ${name}`:source==="FUEL_MAP_USER"&&!id?"User · Anonymous/legacy":role}
function formatPhotoLocation(location:any){if(!location)return "Not checked";if(location.status!=="available")return "Not found in photo metadata";if(location.latitude==null||location.longitude==null)return "GPS detected";return `${Number(location.latitude).toFixed(6)}, ${Number(location.longitude).toFixed(6)}`}
...
function formatLocationMatch(location:any){if(!location)return "Not checked";if(location.status!=="available")return "Cannot match without GPS";const distance=formatDistanceKm(location.nearest_station_distance_km);if(location.match_status==="auto_selected")return distance?`Matched — ${distance} away`:`Matched within ${location.auto_select_radius_km??1} km`;if(location.match_status==="candidate_found_outside_auto_select_radius")return distance?`Candidate found — ${distance} away`:`Candidate found outside auto-match radius`;if(location.match_status==="no_station_within_search_radius")return `No station within ${location.search_radius_km??2} km`;return location.candidate_count?`${location.candidate_count} candidate(s) found`:"No match"}
function formatApplication(job:any){if(!job.applied_at)return "Not applied";return job.requires_confirmation?"Applied after confirmation":"Applied automatically"}

function StationCombobox({stations,value,onChange,loading,error,onRetry}:{stations:any[];value:string;onChange:(value:string)=>void;loading:boolean;error:string;onRetry:()=>void}){
  const activeStations=useMemo(()=>stations.filter(station=>station.is_active!==false),[stations]);const selected=activeStations.find(station=>station.id===value);const [query,setQuery]=useState(selected?.name??"");const [open,setOpen]=useState(false);const [activeIndex,setActiveIndex]=useState(0);const listId="station-search-results";
  useEffect(()=>{setQuery(selected?.name??"")},[value,selected?.name]);
  const matches=useMemo(()=>{const needle=query.trim().toLocaleLowerCase();return activeStations.filter(station=>!needle||[station.name,station.address_line,station.suburb,station.city].some(field=>String(field??"").toLocaleLowerCase().includes(needle))).slice(0,20)},[activeStations,query]);
  function choose(station:any){onChange(station.id);setQuery(station.name);setOpen(false);setActiveIndex(0)}
  return <div className="admin-station-combobox"><label htmlFor="station-search">Station</label>{selected&&<p className="admin-station-current">Selected: <strong>{selected.name}</strong>{stationLocation(selected)&&` — ${stationLocation(selected)}`}</p>}{error&&<div className="admin-inline-error" role="alert"><span>{error}</span><button type="button" onClick={onRetry}>Retry stations</button></div>}<input id="station-search" role="combobox" aria-autocomplete="list" aria-expanded={open} aria-controls={listId} aria-activedescendant={open&&matches[activeIndex]?`station-option-${matches[activeIndex].id}`:undefined} aria-busy={loading||undefined} disabled={loading||Boolean(error&&stations.length===0)} autoComplete="off" placeholder={loading?"Loading stations…":"Search name, address, suburb, or city…"} value={query} onFocus={()=>setOpen(true)} onChange={event=>{setQuery(event.target.value);onChange("");setOpen(true);setActiveIndex(0)}} onKeyDown={event=>{if(event.key==="ArrowDown"){event.preventDefault();setOpen(true);setActiveIndex(index=>Math.min(index+1,Math.max(matches.length-1,0)))}else if(event.key==="ArrowUp"){event.preventDefault();setActiveIndex(index=>Math.max(index-1,0))}else if(event.key==="Enter"&&open&&matches[activeIndex]){event.preventDefault();choose(matches[activeIndex])}else if(event.key==="Escape")setOpen(false)}} />{open&&<div className="admin-station-options" id={listId} role="listbox">{matches.length===0?<p>No matching stations.</p>:matches.map((station,index)=><button id={`station-option-${station.id}`} role="option" aria-selected={station.id===value} className={index===activeIndex?"active":""} type="button" key={station.id} onMouseDown={event=>event.preventDefault()} onClick={()=>choose(station)}><strong>{station.name}</strong><span>{stationLocation(station)||"Location not available"}</span></button>)}</div>}<p className="admin-field-hint">Search results are limited to 20. Add more location detail to narrow the list.</p></div>
}

function stationLocation(station:any){return [station.address_line,station.suburb,station.city].filter(Boolean).join(", ")}

function PriceBoardImage({mediaAssetId,token}:{mediaAssetId:string;token:string}){
  const [imageUrl,setImageUrl]=useState("");const [loadError,setLoadError]=useState("");
  useEffect(()=>{let active=true;let objectUrl="";if(!mediaAssetId){setLoadError("No source photo is associated with this job.");return()=>{active=false}}void fetch(`${api}/admin/media/${encodeURIComponent(mediaAssetId)}/content`,{headers:{authorization:`Bearer ${token}`}}).then(async response=>{if(!response.ok)throw new Error("The uploaded photo could not be loaded.");const blob=await response.blob();if(!blob.type.startsWith("image/"))throw new Error("The uploaded file is not a supported image.");objectUrl=URL.createObjectURL(blob);if(active)setImageUrl(objectUrl);else URL.revokeObjectURL(objectUrl)}).catch(caught=>{if(active)setLoadError(caught instanceof Error?caught.message:"The uploaded photo could not be loaded.")});return()=>{active=false;if(objectUrl)URL.revokeObjectURL(objectUrl)}},[mediaAssetId,token]);
  return <section className="admin-ocr-source" aria-label="Uploaded price-board photo"><h3>Source photo</h3>{!imageUrl&&!loadError&&<p role="status">Loading photo…</p>}{loadError&&<p className="admin-alert" role="alert">{loadError}</p>}{imageUrl&&<img src={imageUrl} alt="Uploaded price board"/>}</section>
}
