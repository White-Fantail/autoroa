...
function formatLocationMatch(location:any){if(!location)return "Not checked";if(location.status!=="available")return "Cannot match without GPS";if(location.match_status==="auto_selected")return `Matched within ${location.auto_select_radius_km??1} km`;if(location.match_status==="candidate_found_outside_auto_select_radius")return `Candidate found ${location.nearest_station_distance_km??""} km away`;if(location.match_status==="no_station_within_search_radius")return `No station within ${location.search_radius_km??2} km`;return location.candidate_count?`${location.candidate_count} candidate(s) found`:"No match"}
