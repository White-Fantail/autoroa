...
export default function AchievementCelebration(){
  ...
  const load=useCallback(async()=>{if(disabled)return;try{const {data}=await supabaseBrowser().auth.getSession();const session=data.session;if(!session)return;const response=await fetch(`${api}/me/achievement-feed`,{headers:{Authorization:`Bearer ${session.access_token}`}});if(!response.ok)return;const body=await response.json();if(Array.isArray(body)&&body.length){setQueue(body);setCurrent(body[0])}}catch{}},[disabled]);
  useEffect(()=>{if(disabled){setQueue([]);setCurrent(null);return;}void load(); const timer=window.setInterval(()=>void load(),45000); return()=>window.clearInterval(timer)},[disabled,load]);
