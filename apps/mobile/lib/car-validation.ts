...
export function selectedStation<T extends {id:string}>(stationId:string|null|undefined,current:T|undefined,results:T[]){
  if(!stationId)return undefined;
  return results.find(station=>station.id===stationId)??(current?.id===stationId?current:undefined);
}
