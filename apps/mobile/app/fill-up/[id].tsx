...
import {fillEditSchema,fuels,selectedStation} from '../../lib/car-validation';
import {formatLocalDateTime,localDateTimeToIso} from '../../lib/fill-up-form';
...
export default function EditFillUp(){
  ...
  const savedStation=useQuery({queryKey:['fuel-station',detail.data?.station_id],queryFn:()=>api.get<{id:string;name:string;address_line:string}>(`/fuel-stations/${detail.data!.station_id}`),enabled:!!detail.data?.station_id});
  const [edit,setEdit]=useState<any>();const [error,setError]=useState('');const [warning,setWarning]=useState<EditFillUpWarning>();const [ack,setAck]=useState<EditFillUpAcknowledgements>(emptyEditFillUpAcknowledgements);const [stationSearch,setStationSearch]=useState('');const [stationResults,setStationResults]=useState<any[]>([]);const [chosenStation,setChosenStation]=useState<any>();const [deleting,setDeleting]=useState(false);const [saving,setSaving]=useState(false);
  useEffect(()=>{if(detail.data){setEdit({...detail.data,occurred_at:formatLocalDateTime(detail.data.occurred_at)});setChosenStation(undefined)}},[detail.data]);
  async function save(acknowledgements:EditFillUpAcknowledgements){const occurredAt=localDateTimeToIso(edit?.occurred_at??'');if(!occurredAt){setError('Enter date and time as YYYY-MM-DD HH:mm.');return}const parsed=fillEditSchema.safeParse({...edit,occurred_at:occurredAt});if(!parsed.success){setError(parsed.error.issues[0]?.message??'Check the editable fields.');return}try{setSaving(true);setError('');await patchEditedFillUp(api.patch,id,parsed.data,acknowledgements);await invalidateEditedFillUpQueries(options=>cache.invalidateQueries(options));router.back()}catch(caught){const next=classifyEditFillUpWarning(caught);if(next){setWarning(next);return}setError(caught instanceof Error?caught.message:'Fill-up could not be saved.')}finally{setSaving(false)}}
  ...
  const currentStation=selectedStation(edit?.station_id,chosenStation??savedStation.data,stationResults);
  return <Screen title="Edit fill-up">
    ...
    {edit&&<Card><Text style={s.muted}>Fields with an outlined box can be edited. Calculated economy values update automatically after saving.</Text>
      {!edit.station_id&&<FormField label="Station search" placeholder="Search station, city, or address" value={stationSearch} onChangeText={setStationSearch} onSubmitEditing={async()=>setStationResults(await api.get<any[]>(`/fuel-stations/search?q=${encodeURIComponent(stationSearch)}`))}/>}
      {edit.station_id&&!currentStation&&savedStation.isLoading&&<Text style={s.muted}>Loading selected station…</Text>}
      {currentStation&&<View style={[s.choice,s.choiceSelected]}><Text>{currentStation.name} · {currentStation.address_line} ✓</Text></View>}
      {!edit.station_id&&stationResults.map(station=><Pressable style={s.choice} key={station.id} onPress={()=>{setChosenStation(station);setEdit((current:any)=>({...current,station_id:station.id}))}}><Text>{station.name} · {station.address_line}</Text></Pressable>)}
      {edit.station_id&&<Pressable style={s.choice} onPress={()=>{setChosenStation(undefined);setStationSearch('');setStationResults([]);setEdit((current:any)=>({...current,station_id:null}))}}><Text>Clear station selection</Text></Pressable>}
      {[['occurred_at','Date and time'],['litres','Litres'],['pump_price_per_litre','Pump price per litre'],['discount_amount','Discount amount'],['total_amount','Total amount'],['odometer_km','Odometer (km)'],['notes','Notes']].map(([key,label])=><View key={key}><FormField label={label} value={String(edit[key]??'')} onChangeText={value=>setEdit((current:any)=>({...current,[key]:value}))}/>{key==='occurred_at'&&<Text style={s.muted}>Shown in your device time zone · YYYY-MM-DD HH:mm</Text>}</View>)}
