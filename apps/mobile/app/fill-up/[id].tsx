...
export default function EditFillUp(){
  ...
  useEffect(()=>{if(detail.data){setEdit({...detail.data});setChosenStation(undefined)}},[detail.data]);
  ...
  return <Screen title="Edit fill-up">
    ...
    {edit&&<Card><Text style={s.muted}>Fields with an outlined box can be edited. Calculated economy values update automatically after saving.</Text>
      ...
      {stationResults.filter(station=>station.id!==currentStation?.id).map(station=><Pressable style={s.choice} key={station.id} onPress={()=>{setChosenStation(station);setEdit((current:any)=>({...current,station_id:station.id}))}}><Text>{station.name} · {station.address_line}</Text></Pressable>)}
      <Pressable style={s.choice} onPress={()=>{setChosenStation(undefined);setEdit((current:any)=>({...current,station_id:null}))}}><Text>Clear station selection</Text></Pressable>
