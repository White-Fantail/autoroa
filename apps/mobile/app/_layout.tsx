...
export default function Root() {
  ...
  useEffect(() => {
    async function routeSession(session: any) {
      ...
      const priorUser=await SecureStore.getItemAsync('carfolio_user_id');const nextUser=session?.user?.id;
      if(priorUser&&priorUser!==nextUser)await SecureStore.deleteItemAsync(`carfolio_fillup_draft:${priorUser}`);
      if(nextUser)await SecureStore.setItemAsync('carfolio_user_id',nextUser);else if(priorUser)await SecureStore.deleteItemAsync(`carfolio_fillup_draft:${priorUser}`);
