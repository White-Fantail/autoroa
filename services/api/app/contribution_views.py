...
def my_contributions(
    status: str = Query("ALL", pattern="^(ALL|REVIEWING|APPLIED|NO_POINTS|FAILED)$"),
...
def contribution_summary(p: Principal = Depends(current_principal), db: Session = Depends(get_db)):
    ...
    total_points = db.scalar(select(func.coalesce(func.sum(PointTransaction.points), 0)).where(PointTransaction.user_id == p.profile.id)) or 0
    month_points = db.scalar(select(func.coalesce(func.sum(PointTransaction.points), 0)).where(PointTransaction.user_id == p.profile.id, PointTransaction.created_at >= month_start)) or 0
    submission_count = db.scalar(select(func.count()).select_from(CommunityPriceBoardSubmission).where(CommunityPriceBoardSubmission.user_id == p.profile.id)) or 0
    applied_prices = db.scalar(select(func.count()).select_from(SubmissionFuelResult).join(CommunityPriceBoardSubmission, CommunityPriceBoardSubmission.id == SubmissionFuelResult.submission_id).where(CommunityPriceBoardSubmission.user_id == p.profile.id, SubmissionFuelResult.result == "APPLIED")) or 0
    return {
        "total_points": int(total_points),
        "month_points": int(month_points),
        "submission_count": int(submission_count),
        "applied_price_count": int(applied_prices),
...
def leaderboard(
    period: str = Query("month", pattern="^(month|all_time)$"),
    scope: str = Query("national", pattern="^(national|region|city|station)$"),
    value: str | None = None,
    ...
    for index, row in enumerate(rows, start=1):
        ...
        item = {
            "rank": index,
            "display_name": "You" if user_id == p.profile.id else _alias(user_id),
            "points": int(points),
            "is_current_user": user_id == p.profile.id,
        }
        if user_id == p.profile.id:
            current = item
        if index <= limit:
            ranked.append(item)
    return {
        ...
        "month_started_at": _month_start_utc() if period == "month" else None,
    }
