...
def refinement_cells(cell: CatalogCell) -> list[CatalogCell]:
    """Split a saturated search into four overlapping, smaller searches."""
    ...
    return [
        CatalogCell(cell.latitude + lat_sign * lat_offset, cell.longitude + lng_sign * lng_offset, child_radius, density, depth)
        for lat_sign in (-1, 1)
        for lng_sign in (-1, 1)
    ]
...
def _place_values(place: dict):
    ...
    if not place_id or not name or not address or not isinstance(latitude, (int, float)) or not isinstance(longitude, (int, float)):
        return None
...
def upsert_catalog_place(db: Session, place: dict) -> tuple[Station | None, str]:
    ...
    item = Station(google_place_id=place_id, country_code="NZ", timezone="Pacific/Auckland", is_active=True, **incoming)
    db.add(item)
    return item, "added"
...
def persist_saturated_cell(db: Session, cell: CatalogCell, result_count: int) -> None:
    ...
    if row:
        ...
        db.add(StationCatalogSaturatedCell(
            latitude=latitude,
            longitude=longitude,
            radius_km=radius,
...
def _scan_cell(
    ...
    *,
    ...
    if len(places) >= 20:
        detail = cell.detail(len(places))
        saturated_details.append(detail)
    ...
    for place in places:
        ...
        stats[status] += 1
    if len(places) >= 20 and cell.refinement_depth < MAX_REFINEMENT_DEPTH:
...
def sync_catalog_batch(db: Session, *, start_cursor: int = 0, max_cells: int = 10, search_cell: Callable[[CatalogCell], list[dict]]) -> dict:
    ...
    stats = {"added": 0, "updated": 0, "existing": 0, "invalid": 0, "provider_results": 0, "refinement_cells": 0}
    seen_place_ids: set[str] = set()
    saturated_details: list[dict] = []
    for cell in cells[start_cursor:end_cursor]:
        _scan_cell(db, cell, search_cell=search_cell, seen_place_ids=seen_place_ids, stats=stats, saturated_details=saturated_details)
    db.commit()
    return {
        **stats,
...
def run_full_catalog_sync(*, start_cursor: int = 0, max_cells: int | None = None, sleep_seconds: float = 0.05) -> dict:
    ...
    cursor = start_cursor
    remaining = max_cells
    aggregate = {"added": 0, "updated": 0, "existing": 0, "invalid": 0, "provider_results": 0, "refinement_cells": 0, "saturated_cells": 0}
    saturated_details: list[dict] = []
    while cursor < total and (remaining is None or remaining > 0):
        ...
        logger.info(
            ...
            cursor, total, aggregate["added"], aggregate["updated"], aggregate["saturated_cells"], aggregate["refinement_cells"],
        )
...
def catalog_plan(p: Principal = Depends(admin_principal)):
    ...
    return {
        ...
        "note": "Saturated cells are automatically split into four smaller overlapping searches and persisted for inspection.",
    }
...
def sync_nz_catalog(
    ...
    try:
        return sync_catalog_batch(db, start_cursor=cursor, max_cells=max_cells, search_cell=GoogleStationCatalogClient(settings.google_maps_api_key).search_cell)
