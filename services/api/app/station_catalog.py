...
TARGETED_MAX_REFINEMENT_DEPTH = 7
...
def refinement_cells(cell: CatalogCell) -> list[CatalogCell]:
    ...
    return [
        CatalogCell(
            cell.latitude + lat_sign * lat_offset,
            cell.longitude + lng_sign * lng_offset,
            child_radius,
            density,
            depth,
        )
...
def _covered_by_dense_overlay(cell: CatalogCell) -> bool:
    """Return True when a coarse/base-derived terminal cell is already covered by a dense metro scan."""
    if not cell.density.startswith("base"):
        return False
    return any(
        south <= cell.latitude <= north and west <= cell.longitude <= east
        for south, north, west, east in DENSE_AREAS
    )
...
def _place_values(place: dict):
    ...
    return (
        ...
        latitude,
        longitude,
        components.get("locality") or components.get("postal_town") or "New Zealand",
...
def persist_saturated_cell(db: Session, cell: CatalogCell, result_count: int) -> None:
    latitude = Decimal(str(round(cell.latitude, 6)))
    longitude = Decimal(str(round(cell.longitude, 6)))
    radius = Decimal(str(round(cell.radius_km, 3)))
    row = db.scalar(
        select(StationCatalogSaturatedCell).where(
            StationCatalogSaturatedCell.latitude == latitude,
            StationCatalogSaturatedCell.longitude == longitude,
            StationCatalogSaturatedCell.radius_km == radius,
            StationCatalogSaturatedCell.refinement_depth == cell.refinement_depth,
        )
    )
    ...
    if row:
        ...
        db.add(
            StationCatalogSaturatedCell(
                ...
                refinement_depth=cell.refinement_depth,
                result_count=result_count,
                first_seen_at=now,
                last_seen_at=now,
            )
...
def _scan_cell(
    ...
    *,
    ...
    if cell.refinement_depth > 0:
        stats["refinement_cells"] += 1
    saturated = len(places) >= 20
    if saturated:
        saturated_details.append(cell.detail(len(places)))
        persist_saturated_cell(db, cell, len(places))
    for place in places:
    ...
    can_refine = (
        saturated
        and cell.refinement_depth < max_refinement_depth
        and cell.radius_km * REFINEMENT_RADIUS_FACTOR >= MIN_REFINEMENT_RADIUS_KM
    )
    if can_refine:
        for child in refinement_cells(cell):
            _scan_cell(
                db,
                child,
                ...
                stats=stats,
                saturated_details=saturated_details,
    ...
    *,
    ...
    return {
        **stats,
        ...
        "saturated_cells": len(saturated_details),
        "saturated_cell_details": saturated_details,
...
def _deepest_saturated_cells(db: Session) -> list[CatalogCell]:
    rows = list(
        db.scalars(
            select(StationCatalogSaturatedCell).where(
                StationCatalogSaturatedCell.result_count >= 20
            )
        )
    )
    if not rows:
    ...
    return [
        CatalogCell(
            ...
            float(row.radius_km),
            row.density,
            row.refinement_depth,
        )
        for row in rows
        if row.refinement_depth == max_depth
    ]
...
def run_saturated_refinement(*, sleep_seconds: float = 0.05) -> dict:
    ...
    with SessionLocal() as db:
        ...
        source_depth = max((cell.refinement_depth for cell in roots), default=None)
        covered_roots = [cell for cell in roots if _covered_by_dense_overlay(cell)]
        unresolved_roots = [cell for cell in roots if not _covered_by_dense_overlay(cell)]
        ...
        for index, root in enumerate(unresolved_roots, 1):
            for child in refinement_cells(root):
                _scan_cell(
                    db,
                    child,
                    search_cell=client.search_cell,
                    seen_place_ids=seen_place_ids,
                    stats=stats,
                    saturated_details=details,
                    max_refinement_depth=TARGETED_MAX_REFINEMENT_DEPTH,
                )
            if index % 10 == 0:
                db.commit()
                logger.info(
                    "station_catalog_final_refinement progress=%s/%s added=%s saturated=%s",
                    index,
                    len(unresolved_roots),
                    stats["added"],
                    len(details),
                )
    ...
    terminal = [
        item
        for item in details
        if item["refinement_depth"] >= TARGETED_MAX_REFINEMENT_DEPTH
        or item["radius_km"] * REFINEMENT_RADIUS_FACTOR < MIN_REFINEMENT_RADIUS_KM
    ]
    return {
        **stats,
        "source_depth": source_depth,
        "source_saturated_cells": len(roots),
        "covered_by_dense_overlay": len(covered_roots),
        "searched_source_cells": len(unresolved_roots),
        "saturated_cells": len(details),
        "terminal_saturated_cells": len(terminal),
        "terminal_saturated_cell_details": terminal,
        "minimum_radius_km": MIN_REFINEMENT_RADIUS_KM,
        "targeted_max_refinement_depth": TARGETED_MAX_REFINEMENT_DEPTH,
        "complete": len(terminal) == 0,
    }


def run_full_catalog_sync(
    *,
    start_cursor: int = 0,
    ...
    while cursor < total and (remaining is None or remaining > 0):
        batch_size = min(
            10,
            total - cursor,
            remaining if remaining is not None else 10,
        )
        with SessionLocal() as db:
            result = sync_catalog_batch(
                db,
                start_cursor=cursor,
                max_cells=batch_size,
                search_cell=client.search_cell,
            )
        for key in aggregate:
            aggregate[key] += result[key]
        saturated_details.extend(result["saturated_cell_details"])
        cursor = result["next_cursor"]
        if remaining is not None:
            remaining -= result["processed_cells"]
        logger.info(
            "station_catalog_progress cursor=%s total=%s added=%s updated=%s saturated=%s refined=%s",
            cursor,
            total,
            aggregate["added"],
            aggregate["updated"],
            aggregate["saturated_cells"],
            aggregate["refinement_cells"],
        )
...
def catalog_plan(p: Principal = Depends(admin_principal)):
    ...
    return {
        ...
        "max_refinement_depth": MAX_REFINEMENT_DEPTH,
        "targeted_max_refinement_depth": TARGETED_MAX_REFINEMENT_DEPTH,
        "minimum_refinement_radius_km": MIN_REFINEMENT_RADIUS_KM,
    }


@catalog_router.get("/admin/stations/saturated-cells")
def list_saturated_cells(
    ...
    rows = list(
        db.scalars(
            select(StationCatalogSaturatedCell)
            .order_by(StationCatalogSaturatedCell.last_seen_at.desc())
            .limit(limit)
        )
    )
    return [
...
def sync_nz_catalog(
    ...
    try:
        return sync_catalog_batch(
            ...
            start_cursor=cursor,
            max_cells=max_cells,
            search_cell=GoogleStationCatalogClient(
                settings.google_maps_api_key
            ).search_cell,
        )
    except (httpx.HTTPError, ValueError) as exc:
        raise HTTPException(502, f"Station catalog sync failed: {exc}") from exc
