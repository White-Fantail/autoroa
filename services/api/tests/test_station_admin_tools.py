...
def test_admin_duplicate_groups_and_merge(client, db):
    ...
    merge_response = client.post(
        f"/api/v1/admin/station-duplicates/{canonical.id}/merge",
        params={"duplicate_id": str(duplicate.id)},
        headers=headers,
    )
    assert merge_response.status_code == 200
    assert merge_response.json()["deleted_station_id"] == str(duplicate.id)

    db.expire_all()
    assert db.get(Station, duplicate.id) is None
    assert db.get(Observation, observation.id).station_id == canonical.id
